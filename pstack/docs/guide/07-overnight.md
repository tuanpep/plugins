...
- `/loop` is a host wake mechanism (bundled in Claude Code; see your host's docs for the OpenCode equivalent), not a pstack skill. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses it to re-check the finish condition on events or a heartbeat.
