...
You don't describe your style, because [`/automate-me`](../../skills/automate-me/SKILL.md) reads it from your history. It mines recent active-workspace transcripts for repeated preferences in replies, delegation, verification, code, prose, and process, then asks which patterns are really yours. It drafts `<your-name>-mode/SKILL.md` with YAML frontmatter between `---` markers. The lowercase, hyphenated `name` matches the directory. The `description` tells an agent when to invoke the skill. Save the result to `.opencode/skills/<your-name>-mode/SKILL.md` for a project skill or `~/.config/opencode/skills/<your-name>-mode/SKILL.md` for a personal skill. It runs the draft through [`/unslop`](../../skills/unslop/SKILL.md) and opens a PR from a worktree for review.
...
Writing a skill matches the [Authoring or modifying a skill playbook](../../skills/poteto-mode/playbooks/authoring-a-skill.md), which writes and validates the frontmatter and body, then ships the result through the Opening a PR playbook. The frontmatter has a lowercase, hyphenated `name` that matches the directory and a `description` that tells an agent when to invoke the skill. Keep `description` as one YAML scalar. Write the body as precise, step-by-step instructions. Save the file to `.opencode/skills/<name>/SKILL.md` for a project skill or `~/.config/opencode/skills/<name>/SKILL.md` for a personal skill. Let the playbook enforce those rules instead of writing a `SKILL.md` freehand.
