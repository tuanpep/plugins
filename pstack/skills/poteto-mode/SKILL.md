...
**Choose the pstack wrapper by work tier.** Use `subagent_type: "poteto-research"` for bounded read-only research, file or call-site inventories, test-command discovery, CI-log triage, and concise evidence summaries. Use `subagent_type: "poteto-worker"` for normal implementation, refactoring, tests, and focused review. Use `subagent_type: "poteto-expert"` only for trace-backed performance work, difficult diagnosis after a bounded investigation, subtle concurrency, security or data-loss boundaries, or a one-way-door design choice. `poteto-mode` and legacy `poteto-agent` are compatibility wrappers only. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) must use a valid host agent type. On OpenCode, do not use `generalPurpose`; route read-only work to `poteto-research`, ordinary implementation and review to `poteto-worker`, and high-risk work to `poteto-expert`.
