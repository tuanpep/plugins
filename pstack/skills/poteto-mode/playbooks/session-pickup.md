...
1. Locate the prior trail. Use `opencode session list --format json` and `opencode export` for a local transcript, or use a cloud-agent URL or pushed branch. Keep session access scoped to the current project. If either session command is unavailable, ask the user directly for the resume context, including what shipped, what is pending, and the decisions made, instead of guessing at another location. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
