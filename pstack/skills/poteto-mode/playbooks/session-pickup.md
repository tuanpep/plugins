...
1. Locate the prior trail. A local transcript the way `recall` locates one (Claude Code's `~/.claude/projects/<slug>/`, or OpenCode's `opencode session list`/`opencode export`; do not glob across other projects, that crosses workspace boundaries and reads private chats from unrelated projects), a cloud-agent URL, or a pushed branch. If neither transcript convention is available on this host, skip straight to asking the user directly for the resume context (what shipped, what's pending, the decisions made) instead of guessing at another location. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
