...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript the way `recall` locates one (Claude Code's `~/.claude/projects/<slug>/`, or OpenCode's `opencode session list`/`opencode export`), scoped to the current project only — never another project's transcripts. If neither convention is available on this host, don't guess at another location — ask each candidate directly which files it opened and grade against that plus the shape of the code. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
