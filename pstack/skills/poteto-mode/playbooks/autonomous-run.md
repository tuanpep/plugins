...
2. Pick the wake mechanism available in OpenCode. An event to watch, such as CI, a merge, or a ref advancing, gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
