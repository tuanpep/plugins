...
# Transcripts dir (Claude Code only): ~/.claude/projects/<slugified-repo-path>/*.jsonl.
# OpenCode keeps sessions in a sqlite db (`opencode db path`), not grep-able files, so
# it isn't wired in here; the LAST_CHAT/BUCKET columns degrade gracefully to "-"/"review"
# on OpenCode and on any host with neither convention.
slug=$(printf '%s' "$main_wt" | sed 's#[^A-Za-z0-9]#-#g')
transcripts="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/projects/$slug"
now=$(date +%s)
