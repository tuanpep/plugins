...
Write `~/.pstack/models.conf`, a config file that sets pstack's model per role. The skills read it and fall back to their inline defaults when a line is absent, so this is an override layer, not a requirement.
...
Tell the user the config file was written. pstack skills read it each time they run, so it takes effect immediately, no new session required. Re-running this skill updates it.
