---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill and runs it through unslop, optionally pulling fresh evidence from recent transcripts."
...
This skill orchestrates two others plus an inline authoring step: an inline mining pass (see step 1), inline skill authoring (step 4), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Author the SKILL.md directly. A SKILL.md is YAML frontmatter between `---` markers, followed by an agent-facing instructions body:

- Frontmatter fields: `name` (lowercase, hyphenated, must match the containing directory name) and `description` (the single most important field — it drives auto-invocation, so make it specific enough that an agent can tell exactly when to invoke this skill versus not).
- Frontmatter formatting: keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or line wrapping requires it.
- Body: write clear, step-by-step instructions as agent-facing prose — imperative, precise, no filler. This is a higher bar than human prose, because an unhelpful sentence becomes an instruction some future agent follows.
...
- User wants a task-specific skill (not working conventions): author it directly using the SKILL.md structure in step 4, no mining required.
